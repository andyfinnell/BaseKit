...
public struct HTTPFormFile: Sendable, Hashable, Codable {
...
struct MultipartFormDataEncoder {
    func encode<E: Encodable>(_ encodable: E) throws -> MultipartFormDataBody {
...
private class MultipartFormData {
    ...
    func setString(_ value: String?, forPath codingPath: [CodingKey]) {
        let key = self.key(from: codingPath)
        let value = value ?? ""
        
        bodyData.append(Data("--\(boundary)\r\n".utf8))
        bodyData.append(Data("Content-Disposition: form-data; name=\"\(key)\"\r\n\r\n".utf8))
        bodyData.append(Data("\(value)\r\n".utf8))
    }

    func setFile(_ formFile: HTTPFormFile, forPath codingPath: [CodingKey]) {
        let key = self.key(from: codingPath)

        bodyData.append(Data("--\(boundary)\r\n".utf8))
        bodyData.append(Data("Content-Disposition: form-data; name=\"\(key)\"; filename=\"\(formFile.filename)\"\r\n".utf8))
        bodyData.append(Data("Content-Type: \(formFile.mimeType)\r\n\r\n".utf8))
        bodyData.append(formFile.data)
        bodyData.append(Data("\r\n".utf8))
    }

    func finalize() -> MultipartFormDataBody {
...
private struct ItemEncoder: Encoder {
    ...
    init(data: MultipartFormData, codingPath: [CodingKey]) {
        self.data = data
...
private class ItemKeyedContainer<K: CodingKey>: KeyedEncodingContainerProtocol {
    ...
    func encode<T: Encodable>(_ value: T, forKey key: K) throws {
        if let file = value as? HTTPFormFile {
            data.setFile(file, forPath: codingPath + [key])
        } else {
